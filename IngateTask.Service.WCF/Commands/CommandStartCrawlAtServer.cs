...
namespace IngateTask.Service.WCF.Commands
{
    ...
    {
        public CommandStartCrawlAtServer(ILogProvider logProvider, ClientConsole clientConsoleLink) : base(logProvider,
            clientConsoleLink)
        {
        }
        ...
        public override bool PropertySetter(object obj)
        {
            if (ClientConsoleLink.OutPutPath.Length != 0)
            {
                try
                {
                    ...
                    ConsoleWCFDispatcher consoleWcfDispatcher = new ConsoleWCFDispatcher(ClientConsoleLink.OutPutPath,
                        ((WCFConsoleClient) ClientConsoleLink)._crawlerCallBack);
